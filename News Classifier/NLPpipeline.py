...
def import_data(data_path,df_train_csv):
...
def Clean_up(df_train,text,use_regex,regex_used,stemming,lemmatization):
...
def feature_engineering(df_train,category_codes):
	...
	* `max_features`: If not None, build a vocabulary that only consider the top
	...
	return([df_train,X_train,y_train,X_test,y_test,features_train,labels_train,features_test,labels_test])
...
def sample_run(sample,choosen_model,text,category_codes,use_regex,regex_used,stemming,lemmatization) :
	...
	sample.columns=[text]
	sample = Clean_up(sample,text,use_regex,regex_used,stemming,lemmatization)
...
def run(data_path,text,use_regex,regex_used,stemming,lemmatization,category_codes,df_train_csv,sample,choosen_model):

	df_train = import_data(data_path,df_train_csv)

	# EDA(df_train)

	df_train = Clean_up(df_train,text,use_regex,regex_used,stemming,lemmatization)

	[df_train,X_train,y_train,X_test,y_test,features_train,labels_train,features_test,labels_test] = feature_engineering(df_train,category_codes)

	for model_to_use in  ['GradientBoostingClassifier','LogisticRegression','KNeighborsClassifier','SVC','RandomForestClassifier']:
	...
	sample_run(sample,choosen_model,text,category_codes,use_regex,regex_used,stemming,lemmatization)
...
def main():
	...
	# big_pickle = glob.glob('./df_train_processed_X_train_y_train_X_test_y_test_features_train_labels_train_features_test_labels_test.pickle')
	choosen_model = 'SVC'

	run(data_path,text,use_regex,regex_used,stemming,lemmatization,category_codes,df_train_csv,sample,choosen_model)
